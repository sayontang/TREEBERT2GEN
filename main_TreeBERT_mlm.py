...
class TreeBERT2_mlm(nn.Module):

    def __init__(self, config):
        super( TreeBERT2_mlm, self).__init__()
        self.config = config
        self.encoder = Tree_BERT(self.config)
        BERT_mlm = BertForMaskedLM.from_pretrained('bert-base-uncased')
        self.encoder.load_state_dict(BERT_mlm.bert.state_dict(), strict = False);
        self.clf = cc(BERT_mlm.cls)
        del BERT_mlm
    
    def forward(self, src_text_tok, src_text_mask, src_event_loc, src_event_pad_mask):
        ...
        decoder_out = self.clf(encoded_src['hidden_states'] ) 
        return  decoder_out
...
def test(model, te_loader, device):
    ...
    for idx, batch in enumerate(te_loader):
        ...
        # forward
        decoder_out = model(src_text_tok = mlm_input, src_text_mask = text_mask_src, 
...
if __name__ == '__main__':  
    ...
    for file_name in ip_data_files:
        ...
        for idx, batch in enumerate(tr_loader):
            ...
            text_mask_src = text_mask_src[:, :max_tok_len_in_batch]
            mlm_input = mlm_input[:, :max_tok_len_in_batch]
            mlm_target = mlm_target[:, :max_tok_len_in_batch]
            #==============================================================================================#
            '''---------------xx Dynamic batch size --> creating the required variables xx---------------'''
            #==============================================================================================#
            ...
            if steps % args.eval_step == 0:
                del text_tok_src, text_mask_src, 
                ...
                loss_profile = {'training_running_loss': tr_running_loss,
                                ...
                                'validation_running_acc': val_running_acc,
                                'logging_step_list': logging_step_list}

                torch.save(loss_profile, './model_checkpoints/TREEBERT_MLM/loss_profile_300k.pt')
                ...
                if val_loss < best_val_loss:
                    ...
                    save_checkpoint(model= model, optimizer= optimizer, scheduler= scheduler, 
                                    ...
                                    path= './model_checkpoints/TREEBERT_MLM/best_val_model_300k.pt' )
